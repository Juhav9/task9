...
using DeviceManager.Areas.Management.Controllers;
...
public class UnitTest : AppTestBase, IClassFixture<WebApplicationFactoryFixture<Program>>
{
        ...
        _client = factoryFixture.WithWebHostBuilder(builder =>
        {
            builder.ConfigureTestServices(services =>
            {
                services.AddAuthentication("Test")
                    .AddScheme<AuthenticationSchemeOptions, TestAuthHandlerManager>(
                        "Test", options => { });
            });
        })
    ...
    public async void Checkpoint02_Anonymous(string data, Tuple<int, string> expected)
    {
        // Act
    ...
    [Fact]
    public void Checkpoint02_UserDevicesController()
    {
    ...
    [Fact]
    public void Checkpoint02_HomeController()
    {
        ...
        Assert.IsAssignableFrom<AuthorizeAttribute>(attr);
        var aa = (AuthorizeAttribute)attr;
        Assert.Equal("Managers", aa.Roles);
        var attr2 = Attribute.GetCustomAttribute(t, typeof(AreaAttribute));
        Assert.NotNull(attr2);
        var ar = (AreaAttribute)attr2;
        Assert.Equal("Management", ar.RouteValue);
    }

    [Theory]
    [JsonFileData("testdata.json", "users", typeof(string), typeof(Tuple<IEnumerable<string>, int, int>))]
    public async Task Checkpoint02_Users(string data, Tuple<IEnumerable<string>, int, int> expected)
    {
        // Arrange
        ...
        Assert.All(expected.Item1, e => Assert.Contains(rowContent, r => r.Contains(e, StringComparison.OrdinalIgnoreCase)));
    }

    [Theory]
    [JsonFileData("testdata.json", "devices", typeof(string), typeof(Tuple<IEnumerable<Dictionary<string, string>>, int, int>))]
    public async Task Checkpoint02_Devices(string data, Tuple<IEnumerable<Dictionary<string, string>>, int, int> expected)
    {
        // Arrange
    ...
    public async Task Checkpoint02_CRUDDevice(Tuple<string, string> data, Tuple<int> expected)
    {
        // Arrange
        ...
        //Act
        var response = await _client.GetAsync($"{data.Item1}/create?userId={data.Item2}");
        ...
        // Act 4
        var responseGetEdit = await _client.GetAsync($"{data.Item1}/edit/{dbDevice.Id}?userId={data.Item2}");
        ...
        // Act 6
        var responseGetDelete = await _client.GetAsync($"{data.Item1}/delete/{dbDevice.Id}?userId={data.Item2}");
        ...
        var hiddenInputs = contentDelete.QuerySelectorAll("input[type=hidden]");
        Assert.NotNull(hiddenInputs);
        Assert.NotEmpty(hiddenInputs);
